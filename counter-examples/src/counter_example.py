...
def TDC(state, weights, LLS_solution, step_size_w, step_size_v):
    ...
    weights += step_size_w * importance_sampling_ratio * (
                TD_error * features[state, :] - discount * features[next_state, :] * np.dot(features[state, :],
                                                                                            LLS_solution))

    # Update LLS solution according to the Least Mean Square (LMS) rule (equation on page 279, under the Equation (11.28))
    LLS_solution += step_size_v * importance_sampling_ratio * (
            TD_error - np.dot(features[state, :], LLS_solution)) * features[state, :]
